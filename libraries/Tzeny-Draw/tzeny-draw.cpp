...
tzeny_draw::tzeny_draw()
{
  ...
  Serial.begin(9600);
  Serial.print("Hello! AIA Tetris Draw manager initializing");
...
void tzeny_draw::drawScore(uint32_t score, uint8_t level)
{
  tft->fillRect(0, 0, blockSize*6, blockSize*7, ST7735_BLACK);

  char buffer[10];

  drawText(0,0,1,"Score");
  sprintf(buffer, "%lu", score);
  drawText(0,2,1, buffer);

  drawText(0,4,1,"Lv");
  sprintf(buffer, "%d", level);
  drawText(0,6,1, buffer);
}
...
void tzeny_draw::drawEndMenu(uint32_t score, uint8_t level) 
{
  current_menu_items = 0;

  initMenu();
  char buffer[10];
  drawText(2,2,2,"GAME OVER");

  drawText(4,7,2,"Score");
  sprintf(buffer, "%lu", score);
  drawText(4,10,2, buffer);

  drawText(4,13,2,"Level");
  sprintf(buffer, "%d", level);
  drawText(4,16,2, buffer);
}
...
//---------------------------HIGHSCORE-------------
void tzeny_draw::addToHighscore(char * p, uint32_t score, uint8_t level)
{
  Serial.println("Adding to highscore!");
  readHighscores();
  sortHighscores();
  ...
  if(highscores_number >= 7)
  {
    highscores_number=7;
    Serial.println("Max number of highscore!");
    if(highscores[6].score > score)
    {
      Serial.println("Too low highscore!" + String(highscores[6].score) + "\\" + String(score));
      return;
    }
    else
    {
  ...
  highscores[highscores_number] = r;
  highscores_number++;
  if(highscores_number>7)
    highscores_number = 7;
